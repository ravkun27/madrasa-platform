...
import { FiChevronDown, FiEdit, FiTrash2, FiVideo, FiFileText, FiCheckSquare } from "react-icons/fi";
...
export const Section = ({ section, courseId ,sectionNum}: { section: any, courseId: any,sectionNum:Number }) => {
    ...
    const toggleSection = (sectionId: string) => {
       ...
       const editSectionName = async (newName: string | null = null, newDes: string | null = null) => {
        try {
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <span className="text-sm font-medium text-indigo-600">{`Post-${sectionNum}`}
              </span>
              {editingSectionId === section._id ? (
                <div className="space-y-2 mt-2">
                  <input
                    ...
                    onBlur={(e) => editSectionName(e.target.value)}
                    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500"
                  />
                  <textarea
                    defaultValue={section.description}
                    onBlur={(e) => editSectionName(null, e.target.value)}
                    className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500"
                  />
                </div>
              ) : (
              ...
              >
                ...
                <div className="space-y-3">
                  {section?.lessonIds?.map((contentId: any) => (
                    <Content key={contentId} contentId={contentId} sectionId={section._id} courseId={courseId} />
                  ))}
                </div>
              </motion.div>
            )}
